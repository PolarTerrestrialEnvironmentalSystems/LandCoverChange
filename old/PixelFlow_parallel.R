...
### Project folder
project_dir <- "bioing/data/LandCoverChange/Run_March2024"
...
invisible(foreach(run = 1:10,
                  .export = c(
                    'project_dir', 'rda_dir', 'data_dir'),
                  .packages = c(
                    'tidyr', 'dplyr',
                    'tibble', 'readr',
                    'readxl', 'sf',
  ...
  }, timeout = 6*60*60, TimeoutException = function(ex) {
    NULL
  }, onTimeout = "silent")
  
  if(is.null(timeout)) {
    png(glue::glue("{dir_out}/summaryResults/timeout.png"))
    plot(1:10, 1:10, type = "n", xaxt = "n", yaxt = "n", xlab = "", ylab = "")
    text(5,5, "Error Message: \n Timeout", cex = 2)
    dev.off()
  }
